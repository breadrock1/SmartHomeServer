...
pub async fn send_request_async(stream: &mut net::TcpStream, str_data: String) -> SendResult {
...
pub fn send_request(stream: &mut std::net::TcpStream, str_data: String) -> SendResult {
    let output_buffer = str_data.as_bytes();
    let buffer_length = output_buffer.len() as u32;
...
pub async fn recv_request_async(stream: &mut net::TcpStream) -> RecvResult {
...
pub fn recv_request(stream: &mut std::net::TcpStream) -> RecvResult {
    let mut input_buffer = [0; 4];
    let _ = stream
        .read(&mut input_buffer)
        .map_err(|e| RecvError::ReadData(e.to_string()));
